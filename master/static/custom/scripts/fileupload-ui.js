...
function success_count() {
    var total_trs = $("#table_upload_file").find("tr").length - 1;
    var success_trs = 0;
    // var success_trs = $("td .success_upload_count").length; //这里success_trs从0开始计数    
    for (var file in jQuery.Device_Flash_Fireware_Global_Args["GLOBAL_IS_FILE_UPLOAD_SUCCESS"]){
        if (jQuery.Device_Flash_Fireware_Global_Args["GLOBAL_IS_FILE_UPLOAD_SUCCESS"][file] === true){
...
(function(factory) {
    ...
    $.widget('blueimp.fileupload', $.blueimp.fileupload, {
            ...
            // Callback for successful uploads:
                ...
                if (data.context) {
                    data.context.each(function(index) {
                        ...
                        jQuery.Device_Flash_Fireware_Global_Args["GLOBAL_IS_FILE_UPLOAD_SUCCESS"][file.webkitRelativePath] = true;
                    ...
                    // if (data.paramName !== undefined) {
                        ...
                        if (data.context) {
                            data.context.each(function(index) {
                                ...
                                if (jQuery.Device_Flash_Fireware_Global_Args["GLOBAL_IS_FILE_UPLOAD_SUCCESS"][data.files[0].webkitRelativePath] === true){
                                    var file = {
                                        status: "上传成功",
        ...
        _startHandler: function(e) {
            ...
            if (jQuery.Device_Flash_Fireware_Global_Args["GLOBAL_ALLFILEUPLOAD_FLAG"] === false){
                ...
                if (jQuery.Device_Flash_Fireware_Global_Args["GLOBAL_FILECANUPLOAD_FLAG"] === false){
                    data.files[0].start = "True"; //作用为文件md5相同后，浏览器不发送文件，点击取消按钮后，触发fail事件，通过这个start属性来给文件添加file.status
                    data.files[0].cancelButton=cancelButton;//方便点击上传按钮后，实际不发送文件，然后点击取消按钮，触发fail事件
                    data.files[0].file_path=file_path;//某种情况下，文件对象传递到_onsend函数里会丢失file.webkitRelativePath，file_path可以代替丢失的属性
                    button.prop('disabled', true);
